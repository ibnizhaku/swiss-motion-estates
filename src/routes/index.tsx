        ...
        <Stagger className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 border border-white/10">
          {stats.map(([n, l]) => (
            <motion.div
              key={l}
              variants={itemVariants}
              className="bg-[#1a1a1a] px-6 py-16 lg:py-24"
            >
              <div className="font-serif text-6xl lg:text-7xl">{n}</div>
              <div className="mt-6 text-[11px] uppercase tracking-widest-xl text-white/60">{l}</div>
            </motion.div>
          ))}
        </Stagger>
      </Section>
